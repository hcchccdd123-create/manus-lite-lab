...
class CodexProvider:
    ...
    async def chat(self, req: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            raise ProviderTimeoutError('Codex timeout') from exc
        except httpx.HTTPStatusError as exc:
            raise self._map_http_error(exc.response) from exc
        except httpx.HTTPError as exc:
            raise ProviderUnavailableError(f'Codex unavailable: {exc}') from exc
    ...
    async def stream_chat(self, req: ChatRequest) -> AsyncGenerator[ChatChunk, None]:
        ...
        try:
            ...
            raise ProviderTimeoutError('Codex stream timeout') from exc
        except httpx.HTTPStatusError as exc:
            raise self._map_http_error(exc.response) from exc
        except httpx.HTTPError as exc:
            raise ProviderUnavailableError(f'Codex stream unavailable: {exc}') from exc
    ...
    def _map_http_error(self, resp: httpx.Response):
        if resp.status_code in (401, 403):
            return ProviderAuthError('Codex auth error')
        if resp.status_code == 429:
            return ProviderRateLimitError('Codex rate limited')
        return ProviderUnavailableError(f'Codex status={resp.status_code}')
