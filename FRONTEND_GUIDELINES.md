...
- 非流式结束后，如后端返回 `termination_reason`，显示“Thinking 自动终止（超时/重复）”状态标签。
- 草稿态：右侧空白 + 输入框垂直居中。
...
- `Web` 开关状态要透传到 `/api/v1/chat/stream` 的 `enable_web_search` 字段。
- 联网意图识别由后端处理；前端不做“是否需要联网”的业务判定。
