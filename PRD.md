...
## 1. 产品定位
Manus-lite 是一个单用户、本地优先的 AI Chat 客户端。

当前版本聚焦传统聊天能力，在会话隔离、流式输出、记忆连续和自动联网搜索上做到稳定、易用、低复杂度。
...
21. 当前版本仅支持 Chat Runtime：
    - 当前版本只保留 `chat` 模式
    - `agent` 模式暂不纳入本轮交付与验收范围
22. 自动联网搜索：
    - 前端不提供 Web Search 开关
    - 支持联网搜索的模型默认具备 Web Search 能力，由模型自行决定是否调用
    - 不支持联网搜索的模型在命中天气/新闻/行情等时效问题时，后端直接返回标准提示，避免幻觉
23. Thinking 稳定性保护：
...
### Story K：我希望联网搜索由系统自动处理
- Given 我提问“今天股市行情/今天深圳天气”这类实时问题
- When 当前模型支持联网搜索
- Then 系统可自动在需要时使用联网搜索，不需要我手动开启

验收：
- 前端不提供 Web Search 开关
- 支持联网搜索的模型默认可自动使用联网搜索能力
- 模型不支持联网搜索且命中时效问题时，后端直接返回标准提示文案（非前端猜测）
